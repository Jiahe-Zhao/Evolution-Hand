...
parser.add_argument("--run_name", type=str, default=None, help="Stable run name for logging/resume.")
parser.add_argument("--checkpoint_interval", type=int, default=None, help="Checkpoint save frequency override.")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    log_root_path = os.path.join(EVOLUTION_LOG_ROOT, agent_cfg["params"]["config"]["name"])
    log_root_path = os.path.abspath(log_root_path)
    print(f"[INFO] Logging experiment in directory: {log_root_path}")
    # specify directory for logging runs
    log_dir = (
        args_cli.run_name
        or os.environ.get("EVOLUTION_RUN_NAME")
        or agent_cfg["params"]["config"].get("full_experiment_name")
...
def create_finished_folder(run_dir):
    """
    在指定 run 目录中创建一个名为 'finished' 的文件夹。

    Args:
        run_dir (str): 训练 run 目录。
    """
    if run_dir:
        finished_folder = os.path.join(run_dir, "finished")
        os.makedirs(finished_folder, exist_ok=True)
        print(f"'finished' 文件夹已创建于: {finished_folder}")
    else:
        print("未找到有效的 run 目录。")
...
if __name__ == "__main__":
    ...
    # 发送信号表示仿真结束，在表示最新模型的文件夹下面创建一个tag文件
    # create_finished_folder("/home/zjh/Evolution/evolution_tasks/logs/shadow_hand_stone_grdind")
