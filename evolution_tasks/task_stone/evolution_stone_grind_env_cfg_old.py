...
class EvolutionStoneGrindEnvCfg(DirectMARLEnvCfg):
    ...
    # action_spaces ={"right_hand": len(actuated_joint_names)+6, "left_hand":  len(actuated_joint_names)+6}
    ...
    # observation_spaces = {"right_hand": len(actuated_joint_names)*3+len(fingertip_body_names)*13+6+16, "left_hand":  len(actuated_joint_names)*3+len(fingertip_body_names)*13+6+13}  # Observation space from input
    # state_space = 2*(len(actuated_joint_names)*3+len(fingertip_body_names)*13+6)+16+13
