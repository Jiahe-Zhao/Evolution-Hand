...
gym.register(
    id="Isaac-Hand-Cube-Evolution-v0", # 任务名称要保持唯一
    entry_point="isaaclab_tasks.evolution_tasks.task_cube.inhand_manipulation.inhand_manipulation_env:InHandManipulationEnv",  # 定义任务（包括任务目标，奖励啥的，重置啥的）
    disable_env_checker=True, # 禁用环境检查器
    kwargs={
        # 定义对象
        # "env_cfg_entry_point": "isaaclab_tasks.evolution_tasks.task_cube.cube_env_cfg:HandEnvCfg",
        "env_cfg_entry_point": "isaaclab_tasks.direct.evolution_hand_different_demo.demo_hand_env_cfg:DemoHandEnvCfg",
        # "env_cfg_entry_point": "isaaclab_tasks.evolution_tasks.task_cube.task_cube_env_cfg:DemoHandEnvCfg",
        # 提供rl_games 库的PPO（近端策略优化）算法的配置
        # 包括胜利条件，迭代次数，多少次储存，batchsize等
