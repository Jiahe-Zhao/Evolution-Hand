#!/usr/bin/env bash
set -euo pipefail
task="$1"
shift
root="${EVOLUTION_CODE_ROOT:-/home/zjh/Evolution_PC}"
isaac="${ISAACLAB_ROOT:-/home/zjh/IsaacLab}"
output_root="${EVOLUTION_DEMO_ROOT:-${root}/outputs/task_suite}"
mkdir -p "${output_root}"
source "${EVOLUTION_CONDA_BASE:-/home/zjh/miniconda3}/etc/profile.d/conda.sh"
conda activate "${EVOLUTION_CONDA_ENV:-evolution_isaaclab}"
exec "${isaac}/isaaclab.sh" -p "${root}/evolution_tasks/task_suite/demo_task.py" --headless --task "${task}" --output "${output_root}/${task}.mp4" --metrics "${output_root}/${task}.json" "$@"
